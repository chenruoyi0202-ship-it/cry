"""vivo careers scraper.

vivo HQ in 东莞 with Shenzhen office. Filter strictly to 深圳.
"""
...
HOMEPAGE = 'https://hr.vivo.com/'
ATTEMPTS: list[dict] = [
    {
        'url': 'https://hr.vivo.com/portal/api/external/recruitment/jobs/list/V1',
        'body_template': {'pageNum': 1, 'pageSize': 50, 'keyword': '',
                          'cityList': ['深圳'], 'recruitType': 1},
    },
    {
        'url': 'https://hr.vivo.com/api/web/job/list',
        'body_template': {'pageNum': 1, 'pageSize': 50, 'cityName': '深圳'},
    },
    {
        'url': 'https://hr.vivo.com/portal/api/external/jobs/page',
        'body_template': {'pageNum': 1, 'pageSize': 50, 'cityList': ['深圳']},
    },
]
JOB_PAGE_TEMPLATE = 'https://hr.vivo.com/job/{job_id}'
...
def _try_endpoint(session: requests.Session, attempt: dict, page_no: int) -> dict:
    ...
    if code not in (0, 200, '0', '200', None):
        raise RuntimeError(f'vivo {attempt["url"]} code={code} '
...
def _to_job(raw: dict) -> Optional[Job]:
    ...
    return Job(
        id=f'vivo_{job_id}', company='vivo', company_name='vivo',
        ...
        source='hr.vivo.com', description=description,
    )
...
def fetch() -> list[Job]:
    ...
    for attempt in ATTEMPTS:
        try:
            with_retries(lambda: _try_endpoint(session, attempt, 1),
                         label=f'vivo probe {attempt["url"]}', retries=2)
            chosen = attempt
            print(f'  [vivo] using {attempt["url"]}', flush=True)
            break
        except Exception as exc:  # noqa: BLE001
            last_err = exc
            print(f'  [vivo] probe failed: {exc}', flush=True)
    if chosen is None:
        raise RuntimeError(f'all vivo endpoints rejected our payload: {last_err}')
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        print(f'  [vivo] page {page}: raw={len(records)} kept={len(page_jobs)} '
...
if __name__ == '__main__':
    out = fetch()
    print(f'vivo fetched {len(out)} jobs')
