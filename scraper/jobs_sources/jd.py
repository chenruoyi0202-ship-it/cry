"""JD (京东) careers scraper.

JD has a Shenzhen 南山 office (京东数科, 京东健康, 京东云). zhaopin.jd.com
...
ATTEMPTS: list[dict] = [
    {
        'url': 'https://zhaopin.jd.com/api/web/job/search/getJobList',
        'body_template': {'keyword': '', 'pageNo': 1, 'pageSize': 50,
                          'cityList': ['深圳'], 'recruitType': 1},
    },
    {
        'url': 'https://zhaopin.jd.com/api/web/job/list',
        'body_template': {'keyword': '', 'pageNum': 1, 'pageSize': 50,
                          'cityName': '深圳'},
    },
    {
        'url': 'https://careers.jd.com/api/web/job/search/getJobList',
        'body_template': {'keyword': '', 'pageNo': 1, 'pageSize': 50,
                          'cityList': ['深圳']},
    },
]
JOB_PAGE_TEMPLATE = 'https://zhaopin.jd.com/web/job/{job_id}'
...
def _try_endpoint(session: requests.Session, attempt: dict, page_no: int) -> dict:
    ...
    for k in ('pageNum', 'pageNo', 'page', 'current'):
        if k in body:
            body[k] = page_no
    ...
    if code not in (0, 200, '0', '200', None):
        raise RuntimeError(f'jd {attempt["url"]} code={code} '
...
def _extract_list(data: dict) -> list[dict]:
    for path in (('data', 'list'), ('data', 'records'),
                 ('data', 'jobList'), ('data', 'items'),
                 ('result', 'list'), ('result', 'jobList')):
...
def _to_job(raw: dict) -> Optional[Job]:
    location_parts = []
    for key in ('cityName', 'workCity', 'city', 'workPlace'):
        v = safe_get(raw, key)
    ...
    department = (safe_get(raw, 'departmentName') or safe_get(raw, 'businessName')
                  or safe_get(raw, 'department') or safe_get(raw, 'orgName') or '')
    category_raw = (safe_get(raw, 'jobCategory') or safe_get(raw, 'category')
                    or safe_get(raw, 'positionType') or safe_get(raw, 'jobClass') or '')
    parts = []
    ...
    return Job(
        id=f'jd_{job_id}', company='jd', company_name='京东',
        ...
        source='zhaopin.jd.com', description=description,
    )
...
def fetch() -> list[Job]:
    ...
    for attempt in ATTEMPTS:
        try:
            with_retries(lambda: _try_endpoint(session, attempt, 1),
                         label=f'jd probe {attempt["url"]}', retries=2)
            chosen = attempt
            print(f'  [jd] using {attempt["url"]}', flush=True)
            break
        except Exception as exc:  # noqa: BLE001
            last_err = exc
            print(f'  [jd] probe failed: {exc}', flush=True)
    if chosen is None:
        raise RuntimeError(f'all jd endpoints rejected our payload: {last_err}')
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        print(f'  [jd] page {page}: raw={len(records)} kept={len(page_jobs)} '
...
if __name__ == '__main__':
    out = fetch()
    print(f'jd fetched {len(out)} jobs')
