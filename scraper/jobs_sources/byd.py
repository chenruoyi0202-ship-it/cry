"""BYD (比亚迪) careers scraper.

BYD is headquartered in 深圳坪山. Their public job listing is on
recruit.byd.com.cn (and historically also on byd.zhiye.com which uses a
3rd-party Beisen platform). Try a few known shapes; orchestrator handles
...
HOMEPAGE = 'https://recruit.byd.com.cn/'
ATTEMPTS: list[dict] = [
    {
        'url': 'https://recruit.byd.com.cn/api/web/job/list',
        'body_template': {'pageNum': 1, 'pageSize': 50, 'keyword': '',
                          'cityList': ['深圳'], 'recruitType': 1},
    },
    {
        'url': 'https://recruit.byd.com.cn/api/job/list',
        'body_template': {'page': 1, 'size': 50, 'cityName': '深圳'},
    },
    {
        'url': 'https://byd.zhiye.com/portal/api/external/recruitment/jobs/list/V1',
        'body_template': {'pageNum': 1, 'pageSize': 50, 'cityList': ['深圳']},
    },
]
JOB_PAGE_TEMPLATE = 'https://recruit.byd.com.cn/job/{job_id}'
...
def _try_endpoint(session: requests.Session, attempt: dict, page_no: int) -> dict:
    ...
    headers = {'Content-Type': 'application/json',
               'Referer': HOMEPAGE,
               'Origin': 'https://recruit.byd.com.cn'}
    ...
    if code not in (0, 200, '0', '200', None):
        raise RuntimeError(f'byd {attempt["url"]} code={code} '
...
def _to_job(raw: dict) -> Optional[Job]:
    location_parts = []
    for key in ('cityName', 'workCity', 'city', 'workPlace', 'location'):
        v = safe_get(raw, key)
    ...
    if not title:
        return None
    department = (safe_get(raw, 'departmentName') or safe_get(raw, 'orgName')
                  or safe_get(raw, 'businessName') or '')
    ...
    return Job(
        id=f'byd_{job_id}', company='byd', company_name='比亚迪',
        ...
        source='recruit.byd.com.cn', description=description,
    )
...
def fetch() -> list[Job]:
    ...
    for attempt in ATTEMPTS:
        try:
            with_retries(lambda: _try_endpoint(session, attempt, 1),
                         label=f'byd probe {attempt["url"]}', retries=2)
            chosen = attempt
            print(f'  [byd] using {attempt["url"]}', flush=True)
            break
        except Exception as exc:  # noqa: BLE001
            last_err = exc
            print(f'  [byd] probe failed: {exc}', flush=True)
    if chosen is None:
        raise RuntimeError(f'all byd endpoints rejected our payload: {last_err}')
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        print(f'  [byd] page {page}: raw={len(records)} kept={len(page_jobs)} '
...
if __name__ == '__main__':
    out = fetch()
    print(f'byd fetched {len(out)} jobs')
