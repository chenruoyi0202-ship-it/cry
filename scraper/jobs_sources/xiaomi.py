"""Xiaomi (小米) careers scraper.

Xiaomi HQ in Beijing; Shenzhen office mostly hardware / 小米汽车. Filter
strictly to 深圳 location.
"""
...
HOMEPAGE = 'https://hr.xiaomi.com/'
ATTEMPTS: list[dict] = [
    {
        'url': 'https://hr.xiaomi.com/web/api/list/jobs',
        'body_template': {'pageNum': 1, 'pageSize': 50, 'keyword': '',
                          'cityList': ['深圳'], 'recruitType': 1},
    },
    {
        'url': 'https://hr.xiaomi.com/api/recruitment/job/list',
        'body_template': {'page': 1, 'size': 50, 'cityName': '深圳'},
    },
]
JOB_PAGE_TEMPLATE = 'https://hr.xiaomi.com/web/job/{job_id}'
...
def _try_endpoint(session: requests.Session, attempt: dict, page_no: int) -> dict:
    ...
    if code not in (0, 200, '0', '200', None):
        raise RuntimeError(f'xiaomi {attempt["url"]} code={code} '
                           f'msg={data.get("msg") or data.get("message")}')
    return data


def _extract_list(data: dict) -> list[dict]:
    for path in (('data', 'list'), ('data', 'records'), ('data', 'rows'),
...
def _to_job(raw: dict) -> Optional[Job]:
    ...
    return Job(
        id=f'xiaomi_{job_id}', company='xiaomi', company_name='小米',
        ...
        source='hr.xiaomi.com', description=description,
    )
    ...
    for attempt in ATTEMPTS:
        try:
            with_retries(lambda: _try_endpoint(session, attempt, 1),
                         label=f'xiaomi probe {attempt["url"]}', retries=2)
            chosen = attempt
            print(f'  [xiaomi] using {attempt["url"]}', flush=True)
            break
        except Exception as exc:  # noqa: BLE001
            last_err = exc
            print(f'  [xiaomi] probe failed: {exc}', flush=True)
    if chosen is None:
        raise RuntimeError(f'all xiaomi endpoints rejected our payload: {last_err}')

    jobs: list[Job] = []
    for page in range(1, MAX_PAGES + 1):
        page_data = with_retries(lambda: _try_endpoint(session, chosen, page),
                                 label=f'xiaomi p{page}')
        records = _extract_list(page_data)
        page_jobs = [j for j in (_to_job(r) for r in records) if j is not None]
        jobs.extend(page_jobs)
        print(f'  [xiaomi] page {page}: raw={len(records)} kept={len(page_jobs)} '
...
if __name__ == '__main__':
    out = fetch()
    print(f'xiaomi fetched {len(out)} jobs')
